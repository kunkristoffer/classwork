...
const cwdt = [
  528, 583, 661, 725, 812, 897, 1003, 1107, 1221, 1354, 1485, 1635, 1804, 1980,
  2184, 2394, 2621, 2874, 3142, 3272, 3580, 3950, 4350, 4780, 5240, 5730, 6250,
  6800, 7380, 7990, 8310, 8630, 8965, 9300, 9650, 10000, 10365, 10730, 11110,
  11490,
];
...
function updateValue() {
  ...
  const skeletonDamage = inputSkeleton
    ? heartboundDamage * 4
    : heartboundDamage * 3;
  const forbiddenriteDamage = inputForbiddenrite
    ? (inputLife * 0.4 + inputEnergy * 0.25) * (1 - inputChaos / 100)
    : 0;
  const totalDamage = inputCharacter
    ? (skeletonDamage + forbiddenriteDamage) * 2
    : skeletonDamage + forbiddenriteDamage;
